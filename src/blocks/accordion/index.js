...
registerBlockType( metadata.name, {
				...
				<InspectorControls>
					<PanelBody title="レイアウト設定">
						...
						<ToggleControl
							...
							onChange={ ( value ) =>
								setAttributes( { useQuestionLabel: value } )
							}
						/>
						<__experimentalUnitControl
							label="質問のラベル幅"
							value={ questionWidth }
							onChange={ ( value ) =>
								setAttributes( { questionWidth: value } )
							}
						/>
						<ToggleControl
							label="回答のラベルを使用する"
							...
							onChange={ ( value ) =>
								setAttributes( { useAnswerLabel: value } )
							}
						/>
						<__experimentalUnitControl
							label="回答のラベル幅"
							value={ answerWidth }
							onChange={ ( value ) =>
								setAttributes( { answerWidth: value } )
							}
						/>
						<__experimentalUnitControl
							label="パディング"
	...
	save: ( { attributes } ) => {
		...
		return (
			<details { ...blockProps }>
				{ useLabel && (
					<RichText.Content tagName="summary" value={ summaryText } />
				) }
				<div className="tagme-accordion-content">
					<InnerBlocks.Content />
				</div>
